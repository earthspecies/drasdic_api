...
class GaussianMixtureGenerator():
    ...
    def __init__(self, rng, p_comps, comp_means, comp_stds):
        """
...
class SceneGenerator:
    ...
    def __init__(self, scene_generator_config_fp: str, support_duration_sec: float, query_duration_sec: float, sr: int):
        """
    ...
    def compute_metadata(self):
        ...
        for info_fp, weight in zip(self.cfg['background_info_fps'], self.cfg['background_weights']):
        ...
        for info_fp, weight in zip(self.cfg['pseudovox_info_fps'], self.cfg['pseudovox_weights']):
        ...
        # Update file paths to absolute locations
        RIR_info['audio_fp'] = RIR_info['audio_fp'].map(
            lambda x: os.path.join(self.cfg['data_parent_dir'], x)
        )
    ...
    def load_audio(self, fp, target_sr):
        ...
        try:
            ...
            warnings.warn(f"Error loading {fp}, using silence instead", RuntimeWarning)
        ...
        if file_sr != target_sr:
            try:
                ...
                warnings.warn(f"Error resampling {fp}, using silence instead", RuntimeWarning)
                return torch.zeros((16000,))
    ...
    def fill_holes(self, labels, max_hole):
        """
    ...
    def loop_with_crops(self, audio, desired_dur_samples, rng):
        """
        ...
        if dur_audio < 10:
            warnings.warn("Encountered unexpectedly short audio, returning silence.", RuntimeWarning)
            return torch.zeros((16000,))
    ...
    def add_pseudovox_to_clip(self, pseudovox_df, audio, annotations, speed_adjust_rate, 
                               snr_db_generator, rms_background, time_reverse_aug, 
                               timegap_generator, rir, snr_min, rng, label):
        """
        ...
        for _, row in pseudovox_df.iterrows():
            ...
            pseudovox = pseudovox * (rms_background / rms_pseudovox) * (10**(.05 * snr_db))
    ...
    def generate_scene(self, seed : int):
        """
        ...
        background_fps = list(self.background_info[background_source]['info']['audio_fp'].sample(n=4, random_state=seed))
        ...
        scene_info['background_audio_query_domain_shift'] = rng.binomial(1, self.cfg['background_audio_query_domain_shift_p'])
        ...
        pseudolabels_allowed = self.pseudovox_info[scene_info['pseudovox_source']]['allowed_clusters'][scene_info['pseudolabel_column']]
        ...
        # While loop handles the corner case where pseudo-labels appear in the background clips we chose. if so, re-choose pseudolabels
        attempts = 0
        while attempts < 5:
            ...
            pseudovox_to_possibly_include = pvi[pvi[scene_info['pseudolabel_column']].isin(pseudolabels_to_possibly_include)]
            original_audio_clips_represented = list(pseudovox_to_possibly_include['raw_audio_fp'].unique())
            isok = True
            for k in ['support_background_fp', 'support_background_fp_2', 'query_background_fp', 'query_background_fp_2']:
        ...
        scene_info['focal_snr_comp_means'] = rng.uniform(low = self.cfg['pseudovox_snr_mean_low'], high = self.cfg['pseudovox_snr_mean_high'], size=2).tolist()
        scene_info['focal_snr_comp_stds'] = rng.uniform(low = self.cfg['pseudovox_snr_std_low'], high = self.cfg['pseudovox_snr_std_high'], size=2).tolist()
        focal_snr_generator = GaussianMixtureGenerator(rng, scene_info['focal_snr_p_comps'], scene_info['focal_snr_comp_means'], scene_info['focal_snr_comp_stds'])
        ...
        scene_info['focal_timegap_comp_means'] = rng.triangular(left = self.cfg['pseudovox_timegap_mean_low'], mode = self.cfg['pseudovox_timegap_mean_low'], right = self.cfg['pseudovox_timegap_mean_high'], size=2).tolist()
        scene_info['focal_timegap_comp_stds'] = rng.uniform(low = self.cfg['pseudovox_timegap_std_low'], high = self.cfg['pseudovox_timegap_std_high'], size=2).tolist()
        for i in range(2):
            if focal_timegap_is_zero[i]:
                scene_info['focal_timegap_comp_means'][i] = 0
                scene_info['focal_timegap_comp_stds'][i] = 0
        focal_timegap_generator = GaussianMixtureGenerator(rng, scene_info['focal_timegap_p_comps'], scene_info['focal_timegap_comp_means'], scene_info['focal_timegap_comp_stds'])
        ...
        scene_info['nonfocal_snr_comp_means'] = rng.uniform(low = self.cfg['pseudovox_snr_mean_low'], high = self.cfg['pseudovox_snr_mean_high'], size=2).tolist()
        scene_info['nonfocal_snr_comp_stds'] = rng.uniform(low = self.cfg['pseudovox_snr_std_low'], high = self.cfg['pseudovox_snr_std_high'], size=2).tolist()
        nonfocal_snr_generator = GaussianMixtureGenerator(rng, scene_info['nonfocal_snr_p_comps'], scene_info['nonfocal_snr_comp_means'], scene_info['nonfocal_snr_comp_stds'])
        ...
        scene_info['nonfocal_timegap_comp_means'] = rng.triangular(left = self.cfg['pseudovox_timegap_mean_low'], mode = self.cfg['pseudovox_timegap_mean_low'], right = self.cfg['pseudovox_timegap_mean_high'], size=2).tolist()
        scene_info['nonfocal_timegap_comp_stds'] = rng.uniform(low = self.cfg['pseudovox_timegap_std_low'], high = self.cfg['pseudovox_timegap_std_high'], size=2).tolist()
        for i in range(2):
            if nonfocal_timegap_is_zero[i]:
                scene_info['nonfocal_timegap_comp_means'][i] = 0
                scene_info['nonfocal_timegap_comp_stds'][i] = 0
        nonfocal_timegap_generator = GaussianMixtureGenerator(rng, scene_info['nonfocal_timegap_p_comps'], scene_info['nonfocal_timegap_comp_means'], scene_info['nonfocal_timegap_comp_stds'])
        ...
        audio_support = self.load_audio(scene_info['support_background_fp'], int(self.sr * scene_info['support_background_resample']))
        audio_query = self.load_audio(scene_info['query_background_fp'], int(self.sr * scene_info['query_background_resample']))
        
        audio_support_2 = self.load_audio(scene_info['support_background_fp_2'], int(self.sr * scene_info['support_background_resample_2']))
        audio_query_2 = self.load_audio(scene_info['query_background_fp_2'], int(self.sr * scene_info['query_background_resample_2']))
        ...
        if scene_info['support_background_mixup']:
            ...
            audio_support_2 = audio_support_2 * rms_background_audio_support / torch.maximum(rms_background_audio_support_2, torch.full_like(rms_background_audio_support_2, 1e-10))
        ...
        if scene_info['query_background_mixup']:
            ...
            audio_query_2 = audio_query_2 * rms_background_audio_query / torch.maximum(rms_background_audio_query_2, torch.full_like(rms_background_audio_query_2, 1e-10))
        ...
        rms_background_audio_support = torch.std(audio_support)
        audio_query = audio_query * rms_background_audio_support / torch.maximum(torch.std(audio_query), torch.full_like(audio_query, 1e-10))
        
        # Add pseudovox to scene, beginning with POS pseudovox
        for i, label in enumerate([2,0]):
            # Label semantics:
            # 2 = Positive
            # 1 = Unknown
            ...
            pseudovox_rate_support = {2: scene_info['focal_rate_support'], 0: scene_info['nonfocal_rate_support']}[label] # call rate per second
            pseudovox_rate_query = {2: scene_info['focal_rate_query'], 0: scene_info['nonfocal_rate_query']}[label] # call rate per second
            ...
            possible_pseudovox = {2 : focal_pseudovox_to_possibly_include, 0 : nonfocal_pseudovox_to_possibly_include}[label]
            ...
            if len(possible_pseudovox_not_in_query) >= 2:
                pseudovox_support = possible_pseudovox_not_in_query.sample(n=n_pseudovox_support, replace=True, random_state=seed)
            else:
            ...
            audio_support, support_labels = self.add_pseudovox_to_clip(pseudovox_support, audio_support, support_labels, speed_adjust_rate, snr_db_generator, rms_background_audio_support, time_reverse_aug, timegap_generator, rir, scene_info["snr_min"], rng, label)
            
            # Add pseudovox to query audio
            audio_query, query_labels = self.add_pseudovox_to_clip(pseudovox_query, audio_query, query_labels, speed_adjust_rate, snr_db_generator, rms_background_audio_support, time_reverse_aug, timegap_generator, rir, scene_info["snr_min"], rng, label)
        ...
        # Package outputs
        out = {"support_audio" : audio_support,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--support-duration-sec", type=float, default=30., help='Duration of support audio')
    parser.add_argument("--query-duration-sec", type=float, default=10., help='Duration of query audio')
    ...
    generator = SceneGenerator(scene_generator_config_fp = args.cfg_fp, support_duration_sec = args.support_duration_sec, query_duration_sec = args.query_duration_sec, sr = args.sr)
    ...
    for i in range(args.n_scenes):    
        ...
        torchaudio.save(os.path.join(output_dir, f"audio_{i}.wav"), torch.cat([support_audio, query_audio], dim=0).unsqueeze(0), args.sr)
